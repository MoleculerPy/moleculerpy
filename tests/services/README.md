# Test Services for MoleculerPy

This directory contains sample microservices for testing the MoleculerPy CLI command and framework.
...
Use the `moleculerpy` command to run these services:

```bash
# Run all services in this directory
moleculerpy /path/to/tests/services

# Run with custom broker ID
moleculerpy /path/to/tests/services --broker-id my-broker

# Run with custom transporter
moleculerpy /path/to/tests/services --transporter nats://custom-host:4222

# Run with custom log level
moleculerpy /path/to/tests/services --log-level DEBUG
```
