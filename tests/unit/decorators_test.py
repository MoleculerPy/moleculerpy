...
class TestActionDecoratorAdvanced:
    """Test advanced action decorator features (timeout, retries, cache)."""

    def test_action_decorator_with_timeout(self):
        """Test action decorator with custom timeout."""

        @action(name="slow.action", timeout=120.0)
        def slow_llm_action():
            return "LLM response"

        assert slow_llm_action._is_action is True
        assert slow_llm_action._timeout == 120.0
        assert slow_llm_action._name == "slow.action"

    def test_action_decorator_with_retries(self):
        """Test action decorator with retry count."""

        @action(name="flaky.action", retries=5)
        def flaky_action():
            return "eventually works"

        assert flaky_action._is_action is True
        assert flaky_action._retries == 5

    def test_action_decorator_with_cache_ttl(self):
        """Test action decorator with cache TTL."""

        @action(name="cached.action", cache_ttl=300)
        def cached_action():
            return "cached result"

        assert cached_action._is_action is True
        assert cached_action._cache_ttl == 300

    def test_action_decorator_with_all_options(self):
        """Test action decorator with all advanced options."""
        params_schema = {"type": "object"}
        ...
        def full_featured_action():
            return "full featured"

        assert full_featured_action._is_action is True
        assert full_featured_action._name == "full.action"
        assert full_featured_action._params == params_schema
        assert full_featured_action._timeout == 60.0
        assert full_featured_action._retries == 3
        assert full_featured_action._cache_ttl == 120

    def test_action_decorator_defaults_none_for_optional_params(self):
        ...
        assert simple_action._timeout is None
        assert simple_action._retries is None
        assert simple_action._cache_ttl is None

    def test_action_decorator_timeout_zero(self):
        """Test action decorator with zero timeout (no timeout)."""

        @action(timeout=0)
        def no_timeout_action():
            return "infinite wait"

        # Zero should be valid (means no timeout in some systems)
        assert no_timeout_action._timeout == 0

    def test_action_decorator_timeout_float_precision(self):
        """Test action decorator with precise float timeout."""

        @action(timeout=0.5)  # 500ms
        def quick_action():
            return "quick"

        assert quick_action._timeout == 0.5
