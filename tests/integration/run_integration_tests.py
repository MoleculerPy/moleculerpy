...
class IntegrationTestRunner:
    ...
    def start_docker_compose(self):
        ...
        # Start services
        code, out, err = self.run_command("docker compose up -d", cwd=self.test_dir, capture=True)
        if code != 0:
            docker_output = f"{out}\n{err}"
            # CI/dev machines may already have NATS bound on 4222.
            if "port is already allocated" in docker_output.lower():
                if self._is_port_open("127.0.0.1", 4222):
                    self.print_status(
                        "Port 4222 already in use, reusing existing NATS instance",
                        "OK",
                    )
                    return True
            self.print_status(f"Failed to start Docker Compose: {docker_output}", "ERROR")
            return False
    ...
    def start_node_services(self):
        ...
        if self.node_process.poll() is not None:
            _stdout, stderr = self.node_process.communicate()
    ...
    def run_python_tests(self):
        ...
        for test_file in self.test_files:
            ...
            # Run with current interpreter from project root so imports resolve reliably.
